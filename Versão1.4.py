...
def buscar_e_pintar(caminho_arquivo, numero_procurado, aba_selecionada):
        ...
        for row in sheet.iter_rows():
            for cell in row:
                if cell.value == numero_procurado:
                    ...
                    log_text = f"Patrimônio {formatar_patrimonio(numero_procurado)} encontrado na aba '{aba_selecionada}' e pintado em verde."
                    ...
                    for row in sheet.iter_rows():
                        for cell in row:
                            if cell.value == numero_procurado:
                                ...
                                log_text = f"Patrimônio {formatar_patrimonio(numero_procurado)} encontrado na aba '{sheet_name}' e pintado em amarelo."
                                patrimonio_encontrado = True
                                break
                        if patrimonio_encontrado:
                            break
    ...
    if not patrimonio_encontrado:
        log_text = f"Patrimônio {formatar_patrimonio(numero_procurado)} não encontrado em nenhuma aba."
...
def on_entry_change(event):
    ...
    try:
        ...
        log_text, patrimonio_encontrado = buscar_e_pintar(caminho_arquivo, numero_procurado, aba_selecionada)
...
def atualizar_planilha_na_interface(caminho_arquivo, aba_selecionada):
    ...
    if aba_selecionada in workbook.sheetnames:
        ...
        log_text_widget.insert(tk.END, f"A aba '{aba_selecionada}' não encontrada no arquivo.\n", 'nao_encontrado')
...
caminho_arquivo = os.path.join(desktop_path, "junio.xlsx")
...
log_text_widget = tk.Text(log_frame, width=80, height=15, wrap=tk.WORD, yscrollcommand=scrollbar_y.set)
...
scrollbar_y_planilha = tk.Scrollbar(planilha_frame, orient=tk.VERTICAL, command=tree.yview)
